-- Manual Setup Script for Stored Procedures (Legacy MySQL Version)
-- Run this directly in MySQL for older MySQL versions
...
CREATE PROCEDURE UpdatePortfolioStatus(IN p_id INT, IN p_status ENUM('draft', 'published', 'archived'))
BEGIN
...
CREATE PROCEDURE CreateUser(IN p_name VARCHAR(255), IN p_email VARCHAR(255), IN p_password VARCHAR(255), IN p_role ENUM('admin', 'user'))
BEGIN
...
CREATE PROCEDURE UpdateContactStatus(IN p_id INT, IN p_status ENUM('new', 'read', 'replied', 'closed'))
BEGIN
